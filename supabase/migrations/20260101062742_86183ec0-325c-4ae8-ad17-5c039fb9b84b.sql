...
CREATE TYPE public.app_role AS ENUM ('owner', 'admin', 'helper', 'user');
...
-- Function to check if any users exist (for first user = owner logic)
...
BEGIN
  ...
  -- Assign role: first user becomes owner, others become regular users
  IF is_first THEN
    assigned_role := 'owner';
  ELSE
    assigned_role := 'user';
